...
const expList=exps.map((exp,ei)=>`<div class="expense-row"><span class="expense-desc">${esc(exp.desc)}</span><span class="expense-cat">${exp.category}</span><span class="expense-amount">${fmt(exp.amount)}</span><button class="expense-del" onclick="deleteExpense(${di},${ei})">✕</button></div>`).join('');
return`<div style="margin-bottom:12px"><div class="budget-day" style="margin-bottom:4px"><div class="budget-day-name" style="color:${day.color}">D${di+1} ${day.date}</div><div class="budget-day-amount">계획 ${fmt(dayPlan)} / 실제 ${fmt(dayActual)}</div></div>${expList}<div class="expense-add-row"><input class="expense-add-input" id="exp-desc-${di}" placeholder="내용"><input class="expense-add-input" id="exp-amount-${di}" placeholder="금액" type="number" style="max-width:80px"><select class="expense-add-select" id="exp-cat-${di}">${catOptions}</select><select class="expense-add-select" id="exp-cur-${di}" style="display:none"><option value="KRW">₩ KRW</option></select><button class="expense-add-btn" onclick="addExpense(${di})">+</button></div></div>`}).join('');
el.innerHTML=`<div class="budget-title">💸 지출 기록</div>${tabsHtml}<div class="budget-total"><div class="budget-total-amount" style="color:#10B981">${totalActual.toLocaleString('ko-KR')}원</div><div class="budget-total-label">총 실제 지출 · 계획 대비 ${total?Math.round(totalActual/total*100):0}%</div></div>${compareHtml}${dayExpenses}`}}
...
el.innerHTML=`<div class="prep-title">🍊 제주 사투리 / 긴급 번호</div>${tabs}<div style="font-size:10px;color:#64748b;margin-bottom:12px">💡 카드를 탭하면 발음을 들을 수 있어요 (TTS)</div>${sections}`}
...
// ══════════ EXCHANGE RATE — 국내여행이라 비활성화 (호출부 호환용 noop) ══════════
function fetchExchangeRate(){/* noop: 제주는 국내여행 */}
...
// ══════════ LIGHT/DARK MODE ══════════
function toggleTheme(){const isDark=document.body.classList.toggle('dark-mode');localStorage.setItem('travel_jeju_theme',isDark?'dark':'light');document.getElementById('themeToggleBtn').textContent=isDark?'☀️':'🌙'}
function restoreTheme(){try{const legacy=localStorage.getItem('travel_jeju_light_mode');if(legacy!==null)localStorage.removeItem('travel_jeju_light_mode')}catch(e){}const t=localStorage.getItem('travel_jeju_theme');if(t==='dark'){document.body.classList.add('dark-mode');document.getElementById('themeToggleBtn').textContent='☀️'}else{document.getElementById('themeToggleBtn').textContent='🌙'}}
...
let html=`<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>여행 일정표 — 제주</title>
...
// ─── 1쪽: 표지 + 전체 요약 ───
html+=`<div class="pdf-page"><div class="pdf-cover"><div class="pdf-cover-title">🍊 제주 가족여행</div><div class="pdf-cover-sub">2026.06.01 — 06.04 · 3박4일</div><div class="pdf-cover-meta">생성일: ${today}</div></div>`;
html+=`<div class="summary-grid">`;
html+=`<div class="summary-card"><div class="summary-label">총 예상 비용</div><div class="summary-value">${fmt(grandTotal)}</div><div class="summary-hint">${DAYS.length}일 · ${totalStops}건 일정 기준</div></div>`;
...
// ══════════ PACKING LIST ══════════
function loadPackingList(){try{const d=localStorage.getItem('travel_jeju_packing_list');if(d)return JSON.parse(d)}catch(e){}// Build default
...
function savePackingList(list){try{localStorage.setItem('travel_jeju_packing_list',JSON.stringify(list))}catch(e){}}
...
// ══════════ SHOPPING LIST ══════════
function loadShoppingList(){try{const d=localStorage.getItem('travel_jeju_shopping_list');if(d)return JSON.parse(d)}catch(e){}
...
function saveShoppingList(list){try{localStorage.setItem('travel_jeju_shopping_list',JSON.stringify(list))}catch(e){}}
...
function resetShoppingList(){if(!confirm('쇼핑리스트를 초기 상태로 되돌릴까요? (체크/추가 내역 모두 사라집니다)'))return;localStorage.removeItem('travel_jeju_shopping_list');renderPrepView();showToast('쇼핑리스트 초기화')}
...
el.innerHTML=`<div class="prep-title" style="text-align:center;margin-bottom:6px">🍊 제주 가족여행</div>
<div style="text-align:center;margin-bottom:16px"><span style="font-size:36px;font-weight:800;color:${ddayColor}">${ddayText}</span><div style="font-size:10px;color:#64748b;margin-top:4px">2026.06.01 — 06.04 · 3박4일</div></div>
<div class="dash-grid">
...
el.innerHTML=`<div class="prep-title">🆘 긴급 연락처 & SOS</div><div style="padding:10px 14px;border-radius:10px;background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.15);margin-bottom:14px;font-size:11px;color:#f87171;text-align:center;font-weight:600">긴급: <a href="tel:112" style="color:#f87171;font-size:16px;font-weight:800">112</a> · <a href="tel:119" style="color:#f87171;font-size:16px;font-weight:800">119</a> · 관광 안내 <a href="tel:1330" style="color:#60a5fa;font-size:14px;font-weight:800">1330</a></div>${contacts}<div class="sos-section" style="margin-top:16px;border-top:1px solid rgba(255,255,255,.06);padding-top:16px"><div class="sos-section-title">📄 중요 서류 사본</div><div style="font-size:10px;color:#64748b;margin-bottom:10px">사진 URL을 저장하면 오프라인에서도 확인할 수 있습니다</div>${docsHtml}</div>`}
function saveDocPhoto(key){const input=document.getElementById('doc-url-'+key);const url=input?.value?.trim();if(!url){showToast('URL을 입력하세요');return}const docs=JSON.parse(localStorage.getItem('travel_jeju_sos_doc_photos')||'{}');docs[key]=url;localStorage.setItem('travel_jeju_sos_doc_photos',JSON.stringify(docs));renderSOSView();showToast('저장 완료')}
function removeDocPhoto(key){const docs=JSON.parse(localStorage.getItem('travel_jeju_sos_doc_photos')||'{}');delete docs[key];localStorage.setItem('travel_jeju_sos_doc_photos',JSON.stringify(docs));renderSOSView();showToast('삭제 완료')}

// ══════════ TIMEZONE — 국내여행이라 단일 KST 고정 ══════════
const TZ_OFFSETS={KST:9};
function getLocalTimeStr(time,fromTZ,toTZ){return''} // 시차 없음
...
let ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//TravelPlanner//JEJU//KO','CALSCALE:GREGORIAN','METHOD:PUBLISH','X-WR-CALNAME:제주 가족여행'];
...
const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='jeju-trip.ics';a.click();URL.revokeObjectURL(a.href);showToast('캘린더 파일 다운로드 완료')}
...
// 제주 영역 가중치 (viewbox=좌,상,우,하)
const TRIP_VIEWBOX='126.10,33.60,126.95,33.15';
...
// ══════════ TAX REFUND CALCULATOR — 국내여행이라 비활성화 (호환용 noop) ══════════
